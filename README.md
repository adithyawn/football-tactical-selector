# ⚽ Football Tactical Selector

![Football Tactical Selector](https://img.shields.io/badge/Version-1.0-blue.svg) 
![HTML5](https://img.shields.io/badge/HTML5-E34F26?logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?logo=css3&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?logo=javascript&logoColor=black)

## � Live Demo

**🚀 [Launch Football Tactical Selector](https://adithyawn.github.io/football-tactical-selector/)**

**📊 [FM Team Analyzer](https://adithyawn.github.io/football-tactical-selector/fm-team-analyzer.html)** - Advanced team performance analysis tool

## �🎯 Description

Football Tactical Selector adalah aplikasi web interaktif yang membantu pelatih dan penggemar sepak bola dalam memilih strategi taktik yang tepat berdasarkan kondisi pertandingan.

## ✨ Features

### 🎯 Football Tactical Selector
- **🔢 Smart Formation Selector**: Rekomendasi formasi berdasarkan karakteristik tim dan lawan
- **⚡ Real-time Recommendations**: Rekomendasi taktik yang berubah secara real-time
- **🎮 Interactive Interface**: Interface yang mudah digunakan dengan dropdown selector
- **⚡ Auto Fill Test Data (NEW!)**: Tombol untuk auto-fill data testing dengan 8+ skenario predefined
- **📊 Comprehensive Analysis**: Analisis mendalam termasuk:
  - Formasi utama dan alternatif
  - Instruksi taktik spesifik
  - Strategi pressing dan marking
  - Rekomendasi tempo permainan
  - Analisis pemain kunci
  - Set pieces strategy
  - Substitution planning

### 📊 FM Team Analyzer (NEW!)
- **📈 Advanced Performance Metrics**: Analisis mendalam performa tim dengan 20+ metrik
- **🎨 Legend Box System**: Benchmarks visible dengan color-coded borders
- **📚 Academic References**: Semua metrik dilengkapi source citations dari penelitian peer-reviewed
- **🗺️ Vertical Progression Analysis**: 6 zona analisis dari defense ke attack
- **⚽ Formation Analysis**: Rekomendasi formasi berdasarkan 3-back, 4-back, 5-back systems
- **🎯 Tactical Recommendations**: 17+ kategori rekomendasi dengan priority levels
- **📖 Complete Glossary**: Dokumentasi lengkap dengan 500+ baris penjelasan
- **🔬 Research-Based**: Menggunakan data dari UEFA Technical Reports, Sarmento et al., Fernandez-Navarro et al., dan lainnya

## 🎛️ Available Options

### Jenis Lawan
- Tim Kuat (Top 6)
- Tim Seimbang (Mid Table)
- Tim Lemah (Bottom Table)
- Tim Promosi
- Tim Rival

### Formasi Lawan
- 4-3-3, 4-2-3-1, 4-4-2
- 3-5-2, 3-4-3, 5-3-2
- 4-5-1, 4-1-4-1, 3-4-2-1, 4-3-2-1

### Gaya Bermain Lawan
- Possession
- Pressing
- Counter Attack
- Long Ball
- Wing Play
- Defensive

### Situasi Game
- Mengejar Skor
- Mempertahankan Keunggulan
- Skor Imbang

### Karakteristik Tim
- Tim Balanced
- Tim dengan Winger Cepat
- Tim dengan Bek Kuat
- Tim dengan AMC Kreatif
- Tim dengan DMC Solid
- Tim Defensif
- Tim dengan 3 Striker
- Tim Lemah Menyerang
- Tim Sangat Defensif
- Tim Fleksibel

## 🚀 Usage

### Manual Input
1. Pilih jenis lawan yang akan dihadapi
2. Tentukan formasi lawan jika diketahui
3. Pilih gaya bermain lawan
4. Tentukan situasi game saat ini
5. Pilih karakteristik tim Anda
6. Sistem akan memberikan rekomendasi taktik secara otomatis

### Quick Testing (NEW! ⚡)
1. Klik tombol **"⚡ Auto Fill Test Data"**
2. Sistem akan otomatis mengisi semua field dengan skenario testing random
3. Lihat rekomendasi yang muncul
4. Klik lagi untuk skenario berbeda
5. Gunakan **"🔄 Refresh / Reset All"** untuk reset

**Available Test Scenarios:**
- 🔥 Derby Match - Mengejar Skor
- 🛡️ Melawan Tim Kuat - Bertahan
- ⚡ Dominasi vs Tim Lemah
- ⚖️ Pertandingan Seimbang
- 🎯 Tim Promosi - Harus Menang
- ⏱️ Injury Time - Desperate
- 🏆 Final Match - Protect Lead
- 🎲 Random Testing

📖 **[Read Full Documentation](AUTO_FILL_FEATURE.md)**

## 🔧 Technical Features

- **Responsive Design**: Mendukung semua ukuran layar (desktop, tablet, mobile)
- **Modern UI/UX**: Interface yang clean dan modern
- **Real-time Processing**: JavaScript untuk pemrosesan real-time
- **No Dependencies**: Pure HTML, CSS, dan JavaScript (tidak memerlukan framework)
- **Fast Loading**: Optimized untuk loading yang cepat

## 📱 Responsive Design

Aplikasi ini fully responsive dan dapat digunakan di:
- 💻 Desktop
- 📱 Tablet
- 📱 Mobile Phone

## 🎨 UI Components

- **Gradient Backgrounds**: Modern gradient design
- **Interactive Dropdowns**: Smooth dropdown interactions
- **Color-coded Sections**: Berbagai warna untuk setiap kategori informasi
- **Modern Typography**: Font yang mudah dibaca
- **Smooth Animations**: Transisi yang halus

## 🧠 Tactical Database

Aplikasi ini menggunakan database taktik yang komprehensif dengan:
- 100+ kombinasi taktik
- Modern tactical systems (Gegenpress, Tiki-Taka, False 9, dll)
- Counter-formation strategies
- Situational tactics
- Time-based recommendations

## 🎯 Target Users

- ⚽ Football coaches
- 🎮 Football Manager players
- 📊 Football analysts
- 👥 Football enthusiasts
- 🏫 Football students/researchers

## 📊 Data Coverage

- **Formation Systems**: 10+ formasi utama
- **Tactical Variations**: 50+ variasi taktik
- **Opponent Types**: 6 kategori lawan
- **Game Situations**: 7 situasi permainan
- **Team Characteristics**: 11 karakteristik tim

## 🔄 Updates

Regular updates include:
- New tactical systems
- Enhanced recommendations
- UI/UX improvements
- Bug fixes
- Performance optimizations

## 📄 License

This project is open source and available under the [MIT License](LICENSE).

## 🤝 Contributing

Contributions are welcome! Please feel free to submit a Pull Request.

## 📧 Contact

For questions or suggestions, please open an issue on GitHub.

---

**Made with ⚽ for football lovers**